        ...
        // Hook de consola SIEMPRE que inyectes
        console.log = function(message) {{
            window.chrome.webview.postMessage(JSON.stringify({{ type: 'consoleMessage', message: message }}));
        }};
        ...
        ws.onmessage = (event) => {{
          ...
          if(snippets[0] === ""852""){{
            if(snippets[1] === ""6770"") {{
                  time = 0;
                  ball = ""6770"";
console.log(""Estoy usando Luz!"");
                }}
            else if (snippets[1] === ""2401""){{
                  time = 500;
                  ball = ""2401"";
console.log(""No Estoy usando Luz!"");
                }}
          }};
          if (snippets[0] === 'ACTION' && snippets[2] == id && snippets[4] == ""tiró""){{
console.log(""Tire Bala de fuego"");
            on=false;
            if(luz === true){{
              nluz--;
              if(nluz === 0){{
...
console.log(""Alguien mas tiro"");
        ...
        if(event.data.includes(""No tienes Balas"")){{
          ...
          if(snippets[0] === ""ACTION"" && snippets[2] == id && snippets[4] == 'tiró'){{
            if(true){{
              setTimeout(() => {{
console.log(""Cargue Bala de Fuego!"");
...
console.log(""Me ire a otro Planet"");
                activate=0;
                setTimeout(() => {{
                  activate=1;
                  another=0;
