...
uid += navigator_info.userAgent.replace(/\D+/g, '');uid += navigator_info.plugins.length;uid += screen_info.height || '';uid += screen_info.width || '';uid += screen_info.pixelDepth || '';
        ...
        var pos;
        var posi;
        var array = [25010064645373624318910205005115190653736576813662,2501006464537361140005373611401823675720128024];
        var text;
        ...
        var pos1;
        var posi1;
        var ran = parseInt(uid)
        var attkbase;
        var newmsg, newmsg1=0;
        if(array.includes(ran) == true){
        }else{
